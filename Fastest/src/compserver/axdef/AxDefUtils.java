 ...
 */
public class AxDefUtils {

	/**
	 * Returns a Theorem with the specified name
	 * @param name
	 */
	public static Theorem getSynonym(String name)
	{
		SynonymsControl synonymsControl = SynonymsControl.getInstance();
		AbstractIterator<Theorem> synonymsIt = synonymsControl.createIterator();
		Theorem result = null;
		while(synonymsIt.hasNext() && result==null)
		{
			Theorem auxSynonyms = synonymsIt.next();
			if(auxSynonyms.getName().equals(name))
				result = auxSynonyms;
		}
		return result;
	}
	 ...
	 * @param synonymName The name of the theorem
	 ...
	 */
	public static String replaceParameters(String synonymName, List<String> realParams, String predicateType, Map<String,Type> mapFR)
	{
		// We obtain the Theorem from the repository
		Theorem theSynonym = getSynonym(synonymName);
		if(theSynonym==null)
		{
			System.out.println(synonymName+" is not the name of a loaded theorem.");
			return null; // Disparo excepcion???
		}
		// We obtain the predicate of the theorem
		String predicate = "";
		if(predicateType.equals("PredicatesToMatch"))
			predicate = theSynonym.getPredicatesToMatch();
		else if(predicateType.equals("SpecialPredicates")){
			List<SpecialLine> lines = theSynonym.getSpecialLines();
		...
		List<Variable> formalParams = theSynonym.getFormalParamList();
		if(formalParams.size() != realParams.size())
		{
			System.out.println("The cardinality of the real parameters not match with the cardinality of formal parameters of the theorem "+synonymName);
			return null; // Disparo excepcion???
		}
