...
/**
 * An instance of this class represents an strategy of test case generation.
 * This strategy is a variant of the AtomicPredTCaseStrategy that iterates over
 * the size of the finite models from 1 to the maximum value of finite models,
 * given in a configuration file.
 */
public class IncrementalAtomicPredTCaseStrategy implements TCaseStrategy {
    ...
    // An indicator to know if the strategy was created automatically or by
    // the user using setfinitemodel command
     ...
     */
    public IncrementalAtomicPredTCaseStrategy() {
    }
     ...
     */
    public IncrementalAtomicPredTCaseStrategy(int fMSize, int maxEval,
    ...
    public AbstractTCase generateAbstractTCase(Spec spec, TClass tClass) {
        ...
        System.out.println("Trying to generate a test case for the class: "
                + tClassName);
        ...
        for (int indexFMSize = 1; indexFMSize <= fMSize; indexFMSize++) {
            // We obtain the instance of the module that contains the precalculated
            // finite models for atomic predicates
            PreCalculatedFiniteModels preCalculatedFiniteModels =
                    PreCalculatedFiniteModels.getInstance();
            preCalculatedFiniteModels.clear();

            // We use a map structure for store the assignments for each variable
            //that satisfy the atomic predicates. This map will filtering in each iteration
            ...
            Map<VariablesSet, List<Map<RefExpr, Expr>>> signFMMap =
                    new HashMap<VariablesSet, List<Map<RefExpr, Expr>>>();
            ...
            FiniteModelCreator finiteModelCreator = new FiniteModelCreator(
                    exprMap, indexFMSize, size, intFiniteModel, natFiniteModel, basicAxDefs);
            ...
            if (pred == null) {
                TClassFiniteModel auxFiniteModel =
                        new TClassFiniteModel(tClass, finiteModelCreator);
                Map<RefExpr, Expr> assign = auxFiniteModel.next();
                abstractTCase =
                        new AbstractTCaseImpl(tClass.getMyAxPara(), tClass.getSchName(), assign);
                break;
            }
            ...
            for (int k = 0; k < reArrangedPred.size() && result; k++) {
                ...
                if (isFromFatherPred(dadPred, atomicPred) && !setByUser) {
                    isFromFather = true;
                ...
                if (!preCalculated) {
                    //                             System.out.println("Precalculado: NO");
                    if (!isFromFather) {
...
/*
  ...
  */
            ...
            /*if(result){
            found = true;
            System.out.println("Lo encontro con tamaño: "+sz);
            }
            else{
            finiteModelCreator = new FiniteModelCreator(
            exprMap, sz + 1, size, intFiniteModel, natFiniteModel, basicAxDefs);
            }
            }*/
...
*/
                ...
                if (satisfied) {
                    ...
                    Map<RefExpr, Expr> mapComplete = tCFM.next();
                    mapComplete.putAll(map);
                    abstractTCase = new AbstractTCaseImpl(tClass.getMyAxPara(),
                            tClass.getSchName(), mapComplete);
                    break;
                } else {
                    if (trunkEvaluations) {
                        abstractTCase = new AbstractTCaseImpl(null, "");
    ...
    private boolean isIndependentPredicate(Pred atomicPred, List<Pred> classPredicates, DeclList declList, int index) {
