...
public final class Utils {
	...
	public static String printCTCC(String testName, ConcreteTCase ctc){
		StringBuilder ctcString = new StringBuilder();
		ctcString = ctcString.append(ctc.getPreamble() + "\n");
		ctcString.append("main(){\n");
		if(ctc.getInitDeclarations() != null)
			ctcString.append(ctc.getInitDeclarations());

		List<TCaseAssignment> assigs = ctc.getAssigments();
		Iterator<TCaseAssignment> iter = assigs.iterator();
		while(iter.hasNext())
			ctcString = ctcString.append((iter.next()).getRefText() + "\n");
		ctcString = ctcString.append(ctc.getEpilogue() + "\n");
		ctcString.append("}\n");
		return ctcString.toString();
	}
