...
public class ConstantGenerator {
	...
	// Faltaria almacenar en ConstantStore

    public static String PLTypeToString(String type, Expr expr, String implID) throws IllegalArgumentException {
        if (type.equals("byte")) {
            return toByte(expr,implID);
        } else if (type.equals("short")) {
            return toShort(expr,implID);
        } else if (type.equals("int")) {
            return toInt(expr,implID);
        } else if (type.equals("long")) {
            return toLong(expr,implID);
        } else if (type.equals("float")) {
            return toFloat(expr,implID);
        } else if (type.equals("double")) {
            return toDouble(expr,implID);
        } else if (type.equals("char")) {
            return toChar(expr,implID);
        } else if (type.equals("String")) // String is supported as PRIMITIVE TYPE
        {
	    // We store the assignment in the unique instance of ConstantStore
	    // We delete the quotes for the ConstantStore
	    //ConstantStore constantStore = ConstantStore.getInstance();
	    //ConstantStoreKey key = new ConstantStoreKey(implID, SpecUtils.termToLatex(expr));
	    //constantStore.set(key,expr);
            String strExpr = toString(expr, implID);
            return strExpr;
        } else if (type.equals("boolean")) {
            return toBoolean(expr,implID);
        } else {
    ...
    // BYTE
    //--------------------------------------------------------------------------------
    public static String toByte(Expr expr, String implID) {
        Byte ret;
	...
	// We store the assignment in the unique instance of ConstantStore
	ConstantStore constantStore = ConstantStore.getInstance();
	ConstantStoreKey key = new ConstantStoreKey(implID, ret.toString());
	constantStore.set(key ,expr);
        return ret.toString();
    }
    //--------------------------------------------------------------------------------
    ...
    // SHORT
    //--------------------------------------------------------------------------------
    public static String toShort(Expr expr, String implID) {
        Short ret;
        String str = exprToString(expr);
    ...
    // INT
    //--------------------------------------------------------------------------------
    public static String toInt(Expr expr, String implID) {
        Integer ret;
        String str = exprToString(expr);
	...
	// We store the assignment in the unique instance of ConstantStore
	// No se si con esto alcanzara. La idea es que almaceno solo cuando se refina a 
	// entero desde otra cosa que no sea un numero.
	ConstantStore constantStore = ConstantStore.getInstance();
	ConstantStoreKey key = new ConstantStoreKey(implID, ret.toString());
	constantStore.set(key ,expr);
        }
    ...
    // LONG
    //--------------------------------------------------------------------------------
    public static String toLong(Expr expr, String implID) {
        Long ret;
        String str = exprToString(expr);
	...
	// We store the assignment in the unique instance of ConstantStore
	// No se si con esto alcanzara. La idea es que almaceno solo cuando se refina a 
	// long desde otra cosa que no sea un numero.
	ConstantStore constantStore = ConstantStore.getInstance();
	ConstantStoreKey key = new ConstantStoreKey(implID, ret.toString());
	constantStore.set(key ,expr);
        }
    ...
    // FLOAT
    //--------------------------------------------------------------------------------
    public static String toFloat(Expr expr, String implID) {
        String str = exprToString(expr);
    ...
    // DOUBLE
    //--------------------------------------------------------------------------------
    public static String toDouble(Expr expr, String implID) {
        String str = exprToString(expr);
    ...
    // CHAR
    //--------------------------------------------------------------------------------
    public static String toChar(Expr expr, String implID) {
        String str = exprToString(expr);
	...
	// We store the assignment in the unique instance of ConstantStore
	ConstantStore constantStore = ConstantStore.getInstance();
	ConstantStoreKey key = new ConstantStoreKey(implID, ret);
	constantStore.set(key ,expr);
        return ret;
    }
    //--------------------------------------------------------------------------------
    ...
    // STRING
    //--------------------------------------------------------------------------------
    public static String toString(Expr expr, String implID) {
        String str = exprToString(expr);
    ...
    // BOOLEAN
    //--------------------------------------------------------------------------------
    public static String toBoolean(Expr expr, String implID) {
        String ret;
        String str = exprToString(expr);
    ...
    public static String exprToString(Expr expr) {
        ...
        if (expr instanceof NumExpr) {
            ...
            if (leftExpr instanceof RefExpr) {
                ...
                int[] opCode = getOperatorCode(operatorString);
            ...
            Utils.printErrorInvalidFormatArgumentforVariable(expr.toString());
            ret = null;
        }
        //--------------------------------------------------------------------------------
   ...
   //--------------------------------------------------------------------------------    
	...
	// Quedo algo incompleto o la primera linea es innecesaria!!!
        List<NodeElement> fields = structure.getElements();
        return head;
    }
    //--------------------------------------------------------------------------------

    public static String pointerToPLType(String head, NodePLType plType, Expr expr, String implID) {
        head = head.concat(" = ");

        head = head.concat(PLTypeToString(plType.getType(), expr, implID) + ";\n");
