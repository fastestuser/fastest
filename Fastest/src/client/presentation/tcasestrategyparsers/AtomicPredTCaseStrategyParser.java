 ...
 */
public class AtomicPredTCaseStrategyParser implements TCaseStrategyParser{
    ...
    public boolean parse(TClass tClass, String args, TCaseStrategy tcs)
        ...
        try{
            ...
            if(!(tcs instanceof AtomicPredTCaseStrategy))
            ...
            for(int index = 0, argCount=1; index < argc; index += argCount ){
                if(argv[index].equals("-size")){
                    ...
                    atomicPredTCaseStrategy.setFiniteModelSize(size);
                    argCount += 2;
                    sizeSetted = true;
                }
                    ...
                    atomicPredTCaseStrategy.setGivenTypeFiniteModelSize(btSize);
                    argCount += 2;
                    btSizeSetted = true;
                }
                    ...
                    atomicPredTCaseStrategy.setIntNatFiniteModelSize(nzSize);
                    argCount += 2;
                    nzSizeSetted = true;
                }
                    ...
                    for(int k=0; k < finiteModels.length;k++){
                        ...
                        atomicPredTCaseStrategy.setExprMap(exprMap);
                    }
            ...
            if(!sizeSetted)
                atomicPredTCaseStrategy.setFiniteModelSize(3);

            if(!btSizeSetted)
                atomicPredTCaseStrategy.setGivenTypeFiniteModelSize(-1);


            if(!nzSizeSetted)
                atomicPredTCaseStrategy.setIntNatFiniteModelSize(-1);



            atomicPredTCaseStrategy.setIntFiniteModel(intFiniteModel);
            atomicPredTCaseStrategy.setNatFiniteModel(natFiniteModel);

	    atomicPredTCaseStrategy.isSetByUser(true);

            Controller controller = clientTextUI.getMyController();
            int maxEval = controller.getMaxEval();
            atomicPredTCaseStrategy.setMaxEval(maxEval);
	    Map<String, TClassNode> mapTTree = controller.getOpTTreeMap();
	    atomicPredTCaseStrategy.setTTreeMap(mapTTree);
