...
%%
%%          Last revision by Gianfranco Rossi (August 2012)           
%%                                                              
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
write('   2.  Set/multiset/list constraints:'), nl,
   ...
   write('        - nsubset(s1,s2,s3) (not subset)'), nl,
   ...
   write('        - sum(s,n) (sum all elements (non-negative integers))'), nl,
...
h(sbuilt) :-
   ...
   write('   -   prolog_call/1: to call any Prolog goal'), 
        write(' from {log}'),nl,  
   write('   -   consult_lib/0: to consult the {log} library file'),nl,  
   write('   -   !/1 (used postfix): G!, G any {log} goal: '),
        write('makes G deterministic'), nl,
   write('   -   delay(G,C), G, C any {log} goals: delays '),
...
solve_SF(S,GName,_PName,VarList,_Cin,Cout) :- 
        ...
        list_to_set(L,S,C2),
        append(C2,C1,Cout).
%SF solve_SF(S,GName,PName,VarList,Cin,Cout) :-
%SF        solve_RUQ(S,GName,VarList,Cin,C1),
%SF        InPred =.. [PName,S|VarList],
%SF        solve_goal_fin(C1,[neg(InPred & true)],Cout).
...
sys_special(consult_lib,[]):-          %% consult the {log} library file                         
      !,rem_clause(lib), rem_clause(tmp(lib)),             
      setlog_open('setloglib.slog',read,FileStream), 
      switch_ctxt(lib,OldCtxt),
      read_loop_np(FileStream),
      switch_ctxt(OldCtxt,_),
      close(FileStream). 

sys_special(consult(File),[]):-        %% consult                          
      !,setlog_open(File,read,FileStream), 
      write('consulting file '), write(File), write(' ...'), nl,
...
atomic_constr(min(_,_)).             
atomic_constr(max(_,_)).  
...
sat_step([min(X,Y)|R1],R2,Z,F):- !,         
          sat_min([min(X,Y)|R1],R2,Z,F).  
sat_step([max(X,Y)|R1],R2,Z,F):- !,            
          sat_max([max(X,Y)|R1],R2,Z,F).                      
%%%%%%%%%%%%%             % type constraints
...
sat_min([min(S,N)|R1],
        [solved(min(S,N),(var(S),var(N)),1,f)|R2],c,F):-   % inference rule  
        var(S),var(N),!,              % min(S,N) (irreducible form)
        solve_FD(N >= 0),                    
        sat_step(R1,R2,_,F).
sat_min([min(S,_)|_],_,c,_):-         % min({},t) or min(int(a,b),t) with a>b 
        nonvar(S),is_empty(S),!,
        fail.                       
sat_min([min(S,T)|R1],R2,c,F):-       % min({X},T)                       
        nonvar(S),S=R with X,
        nonvar(R),R=={},!,
        T = X,
        simple_integer_expr(T),
        solve_FD(T >= 0),
        sat_step(R1,R2,_,F).       
sat_min([min(I,T)|R1],R2,c,F):-       % min(int(a,b),t) t=a
        nonvar(I),is_int(I,A,_B),!,
%        solve_FD(A=<B),   
        T = A,
        solve_FD(T >= 0),
        sat_step(R1,R2,_,F).
sat_min([min(S,T)|R1],R2,c,F):-       % ground case 
        ground(S),!,                
        simple_integer_expr(T),     
        g_min(S,T),
        sat_step(R1,R2,_,F).
sat_min([min(S,T)|R1],[min(S,T)|R2],Stop,nf):-  % min(S,t), S var., t nonvar
        var(S),!,                     % delayed until final_sat is called
        sat_step(R1,R2,Stop,nf).      % (--> Level 3)
sat_min([min(S,T)|R1],R2,R,f):-       % LEVEL 3: min(S,k) S var., k nonvar      
        var(S),!,                     % (k integer constant)          
        integer(T),        
        solve_FD(T >= 0),
        (sat_step([S=N with X,set(N),integer(X),X nin N,X = T,min(N,M),M > T|R1],R2,R,f)
         ;
         sat_step([S={} with X,integer(X),X = T|R1],R2,R,f)). 
sat_min([min(R with X,T)|R1],[min(R with X,T)|R2],Stop,nf):- 
        bounded(R with X),!, 
        simple_integer_expr(X),
        solve_FD(T >= 0),    
        minim(R with X,T),       
        sat_step(R1,R2,Stop,nf).  
sat_min([min(R with X,T)|R1],[min(R with X,T)|R2],Stop,nf):-!,  % min({...|R},t)
        sat_step(R1,R2,Stop,nf).       % delayed until final_sat is called
sat_min([min(R with X,T)|R1],R2,c,f):- % LEVEL 3: min({...},t)           
        simple_integer_expr(T),
        simple_integer_expr(X),
        solve_FD(T >= 0), 
%        solve_FD(X >= 0), 
        (sat_step([integer(X),set(R),X nin R,X = T,min(R,M),M > T|R1],R2,_,f)
         ; 
         sat_step([integer(X),set(R),X nin R,X > T,min(R,T)|R1],R2,_,f)
        ). 
...
sat_max([max(S,N)|R1],
        [solved(max(S,N),(var(S),var(N)),1,f)|R2],c,F):-   % inference rule   
        var(S),var(N),                % max(S,N) (irreducible form)
        S \== {},!, 
        solve_FD(N >= 0),           
        sat_step(R1,R2,_,F).

sat_max(max({},_),_,c,_):-!,
        fail.                         % max({},t) non definito

sat_max([max(S,T)|R1],R2,c,F):-       % min({X},t)                       
        simple_integer_expr(T),
        nonvar(S),
        S= {} with X,
        simple_integer_expr(X),!,     
...
sat_max([max(I,T)|R1],R2,c,F):-        % max(int(a,b),t) t=b
        nonvar(I), is_int(I,A,B),!,
        solve_FD(A=<B),   
        simple_integer_expr(T),     
        solve_FD(T is B),
...
sat_max([max(S,T)|R1],R2,c,F):-        % ground case
        ground(S),
        S \== {},!,                
        simple_integer_expr(T),     
        g_max(S,H),
...
sat_max([max(S,T)|R1],[max(S,T)|R2],Stop,nf):-  % max(S,t), S var., t nonvar
        var(S),nonvar(T),
        S \== {},!,                    % delayed until final_sat is called 
        solve_FD(T >= 0),              % (--> Level 3)
        sat_step(R1,R2,Stop,nf).       
sat_max([max(S,T)|R1],R2,R,f):-        % LEVEL 3: max(S,k) S var., k nonvar      
        var(S), nonvar(T),!,           % (k integer constant)          
        integer(T),     
        solve_FD(T >= 0),
        (sat_step([S = N with X, set(N), integer(X), X nin N, X >= 0, X is T, max(N,M), M < T|R1],R2,R,f)
         ;
         sat_step([S = {} with X, integer(X), X >= 0, X is T|R1],R2,R,f)). 
     
sat_max([max(R with X,T)|R1],[max(R with X,T)|R2],Stop,nf):- 
        bounded(R with X),  
        integer(T),!,                                             % max({...},t) nf
        solve_FD(T >= 0),    
        mass(R with X,T),       
        sat_step(R1,R2,Stop,nf).  
sat_max([max(R with X,T)|R1],[max(R with X,T)|R2],Stop,nf):-
        simple_integer_expr(T),!,          % max({...|R},t)
        solve_FD(T >= 0),
        sat_step(R1,R2,Stop,nf).   

sat_max([max(R with X,T)|R1],R2,_,f):-     % LEVEL 3: max({...},t)           
         simple_integer_expr(T),
         simple_integer_expr(X),
         solve_FD(T >= 0), 
         solve_FD(X >= 0),   
         (sat_step([set(R), integer(X),X = T,X nin R, max(R,M), M>= 0, M < T|R1],R2,_,f)
         ; 
         sat_step([set(R), integer(X), X < T,X nin R, max(R,T)|R1],R2,_,f)). 
...
is_min(min(X,N),_,X,N) :- !.     
is_min(solved(min(X,N),_,Lev,_),Lev,X,N).

is_max(max(X,N),_,X,N) :- !.     
is_max(solved(max(X,N),_,Lev,_),Lev,X,N).
...
find_min(X,[C|_],N) :-       % min(X,N) is in the CS?   
       is_min(C,_,S,N),
       X == S,!.
find_min(X,[_|R],M) :- 
       find_min(X,R,M).

find_max(X,[C|_],N) :-       % max(X,N) is in the CS?  
       is_max(C,_,S,N),
       X == S,!.
find_max(X,[_|R],M) :- 
       find_max(X,R,M).
...
gen_type_constrs(min(T1,T2),[type(set(T1)),type(integer(T2))]):- !.  
gen_type_constrs(max(T1,T2),[type(set(T1)),type(integer(T2))]).  
