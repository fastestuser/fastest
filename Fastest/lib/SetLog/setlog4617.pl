...
%%
%%          Last revision by Gianfranco Rossi (December 2012)           
%%                                                              
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
h(sbuilt) :-
   ...
   write('   -   consult_lib(F): to consult any file F as a {log} library file'),nl,  
...
sys_special(consult_lib(File),[]):-      %% add clauses in file F to the 
      !,setlog_open(File,read,FileStream), %% {log} library clauses
      switch_ctxt(lib,OldCtxt),
      read_loop_np(FileStream),
      switch_ctxt(OldCtxt,_),
      close(FileStream). 
