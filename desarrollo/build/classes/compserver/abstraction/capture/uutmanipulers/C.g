...
options {
    ...
    memoize=true;
    k=2;
    output = template; // Opcion modificada
    rewrite = true; // Opcion modificada
}
...
@init {
  $Symbols::types = new HashSet();
	...
	:	declaration_specifiers? declarator
		...
		) ->  {$function_definition.text.contains("static ")}? template(funcReplaced={$function_definition.text.replace("static ","")})
		   "/*Funcion reemplazada*/ <funcReplaced>"
		->  template(funcOriginal={$function_definition.text})
		   "<funcOriginal>"
	;
...
@init {
  $declaration::isTypedef = false;
	...
	| declaration_specifiers init_declarator_list? {if($init_declarator_list.underMonitoring)
		System.out.println("Esta siendo monitoreado: "+$init_declarator_list.text);
	}';' ->  {$declaration.text.contains("static ") && $init_declarator_list.underMonitoring}? template(declReplaced={$declaration.text.replace("static ","")})
		   "<declReplaced>"
		->  template(declOriginal={$declaration.text})
		   "<declOriginal>"
	;
...
init_declarator_list returns [boolean underMonitoring]
@init{
	$underMonitoring = false;
}
	: init1 = init_declarator {if($init1.underMonitoring)
				$underMonitoring = true;
			}(',' init2 = init_declarator {if($init2.underMonitoring)
				$underMonitoring = true;
			})*
	;

init_declarator returns [boolean underMonitoring]
	: declarator ('=' initializer)? {$underMonitoring = $declarator.underMonitoring;}
	;
...
declarator returns [boolean underMonitoring]
	: pointer? direct_declarator {$underMonitoring = $direct_declarator.underMonitoring;}
	| pointer {$underMonitoring = false;}
	;

direct_declarator returns [boolean underMonitoring]
	:   (	IDENTIFIER
			{
			...
			$underMonitoring = false;
			for(int i=0;i<monitoredVars.size() && !$underMonitoring;i++)
				if(monitoredVars.get(i).equals($IDENTIFIER.text)){
