...
public class InitializeRegistry {
	...
	if (channel instanceof ServerSocketChannel){
	 ...
	 // calls until the service is bound in the registry.
	RMIServerSocketFactory factory =
	    new RegistryServerSocketFactory(serverSocket);

	// Create registry and bind name to proxy in registry.
	Registry registry = LocateRegistry.createRegistry(port, null, factory);
	try {
     ...
     * from 'xinetd' with 'wait' status.  This socket factory's
