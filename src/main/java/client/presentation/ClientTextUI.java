...
public class ClientTextUI extends ClientUI {
    ...
    public ClientTextUI(BufferedReader input, PrintWriter output) {
        try {
            ...
            (new CServersConfigLoader("/conf/cserversinfo.conf")).loadCServersInfo();
            (new RWRulesLoader("/conf/rwRules.tex")).loadRWRules();
            (new TheoremsLoader("/conf/elimTheorems.tex", "/conf/thmoperators.conf")).loadTheorems();
